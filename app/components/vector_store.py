...
def load_vector_store(): # loading the existed vector store
    try:
...
def save_vector_store(text_chunks): #creating  new vectorstore
    try:
        if not text_chunks:
            raise CustomException('No chunks were found')
        
        logger.info('Generating our new vector store')
